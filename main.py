...
_litellm.drop_params = True  # reasoning models reject temperature/top_p; drop silently
...
def _strip_html(h: str) -> str:
    h = _re.sub(r"(?is)<(script|style|noscript)[^>]*>.*?</\1>", " ", h)
    h = _re.sub(r"(?s)<[^>]+>", " ", h)
    return _re.sub(r"\s+", " ", _html.unescape(h)).strip()

def _fetch_page(url: str, timeout: int = 8) -> str:
    try:
        r = _requests.get(url, timeout=timeout, headers={"User-Agent": "Mozilla/5.0"})
        if r.status_code == 200 and "html" in r.headers.get("content-type", ""):
            return _strip_html(r.text)[:2500]
    except Exception:
        pass
    return ""

def keyless_web_search(query: str, max_results: int = 10, fetch_top: int = 3) -> str:
    """Live DuckDuckGo research, no API key. Returns evidence text, or '' on any failure."""
    ...
    for r in results:
        ...
        lines.append(f"- {r.get('title','')} ({u})\n  {r.get('body','')}")
        if u:
            urls.append(u)
    pages = []
    try:
        with _cf.ThreadPoolExecutor(max_workers=max(1, fetch_top)) as ex:
            for u, text in zip(urls[:fetch_top], ex.map(_fetch_page, urls[:fetch_top])):
                if text:
                    pages.append(f"[{u}]\n{text}")
    except Exception as e:
...
class SummerTemplateBot2026(ForecastBot):
    ...
    async def _web_grounded_research(self, question: MetaculusQuestion, base_prompt: str) -> str:
        ...
        srcs = [l.split("(")[-1].rstrip(")") for l in evidence.splitlines() if l.startswith("- ") and "(" in l]
        if srcs:
            research += "\n\nRetrieved sources:\n" + "\n".join(srcs)
        try:
            cache[key] = {"ts": _time.time(), "research": research}
            _json.dump(cache, open(_RESEARCH_CACHE, "w"))
        except Exception:
            pass
        return research
    ...
    async def run_research(self, question: MetaculusQuestion) -> str:
        async with self._concurrency_limiter:
            ...
            if isinstance(researcher, GeneralLlm):
                ...
                research = await self._web_grounded_research(question, prompt)
...
if __name__ == "__main__":
    ...
    template_bot = SummerTemplateBot2026(
        ...
        llms={
            # Azure Foundry gpt-5.6-sol via litellm. Env: AZURE_API_KEY /
            # AZURE_API_BASE / AZURE_API_VERSION (litellm reads them directly).
            # ponytail: model string set from probe_azure.py — the working one.
            "default": GeneralLlm(
                model=os.environ.get("BOT_LLM", "azure/gpt-5.6-sol"),
                # gpt-5.6-sol is a reasoning model: rejects temperature != default
                timeout=120,
                allowed_tries=2,
            ),
            "summarizer": os.environ.get("BOT_LLM", "azure/gpt-5.6-sol"),
            # ponytail: LLM-only research (no news!) until AskNews creds
            # are added — set ASKNEWS_CLIENT_ID/SECRET + "asknews/news-summaries".
            "researcher": os.environ.get("BOT_LLM", "azure/gpt-5.6-sol"),
            "parser": os.environ.get("BOT_LLM", "azure/gpt-5.6-sol"),
        },
    )
